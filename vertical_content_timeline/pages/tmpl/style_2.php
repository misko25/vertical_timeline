...
<div class="item" data-id="<?php echo $arr['dataid'] ?>" <?php $my_name=(($arr['node-name'] && $arr['node-name'] != '') ? ' data-name="'.$arr['node-name'].'"': ''); echo $my_name;?> data-description="<?php echo mb_substr($arr['item-title'],0,30);?>">
	...
	?>
		...
		<div class="item_open <?php ($arr['item-image'] == '' ? ' item_open_noImg' : '') ;?>" data-id="<?php echo $arr['dataid'];?>" <?php $var=(!isset($tpreview) ? 'data-access="'.admin_url( 'admin-ajax.php' ).'?action=vctimeline_frontend_get&vtimeline='.$id.'&id='.$key.'"': '');echo $var;?>>
...
if(!isset($tpreview)) {
	...
	if ($arr['item-open-image'] != '') {
					?>
			<a class="vertical_timeline_rollover_bottom vertical_con_borderImage" href="<?php (($arr['item-open-prettyPhoto'] != '')? $arr['item-open-prettyPhoto'] : $arr['item-open-image']);?>" rel="prettyPhoto[timeline]">'
	<?php
	$image = '';
	if($arr['item-image'] != '') {
	...
	<?php
			...
			?>
			<div class="vertical_timeline_open_content" style="height:<?php $my_w_12 =(intval($settings['item-height']) - 2*intval($settings['item-open-content-padding'])).'px';echo $my_w_12;?>">
			<?php
				}
